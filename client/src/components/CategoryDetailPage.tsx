...
export function CategoryDetailPage({
  ...
  const renderCompanyCard = (company: Company) => {
    ...
    return (
      <Card key={company.id} className="p-4 hover:shadow-medium transition-all cursor-pointer border-slate-200 rounded-2xl hover-elevate active-elevate-2" data-testid={`card-company-${company.id}`}>
        <div
      ...
      <div className="bg-white px-6 py-8 border-b border-slate-200 sticky top-0 z-10 shadow-soft">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1">
              <h1 className="mb-1 text-slate-900" data-testid="text-category-name">{category.name}</h1>
        ...
        {user && (
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="p-3 bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl border border-purple-100">
              <p className="text-xs text-purple-900 mb-1">Stores Tracked</p>
              <p className="text-lg text-purple-900" data-testid="text-stores-tracked">{userSpendingInCategory.length}</p>
            </div>
            <div className="p-3 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl border border-emerald-100">
              ...
              <div className="flex items-center gap-2">
                <p className={`text-lg ${getScoreColor(categoryScore)}`} data-testid="text-avg-score">{categoryScore}</p>
